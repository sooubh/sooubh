...
export const GitHubActivityPanel: React.FC<{ compact?: boolean }> = ({ compact = false }) => {
  ...
  const renderedEvents = useMemo(() => events.slice(0, compact ? 3 : 6), [events, compact]);

  return (
    <section className={compact ? "mt-6 space-y-4" : "mt-16 space-y-6"} id="github-live-activity">
      <div className={`grid grid-cols-2 gap-3 ${compact ? "md:grid-cols-2" : "md:grid-cols-4"}`}>
      ...
      <div className={`grid gap-6 ${compact ? "grid-cols-1" : "lg:grid-cols-12"}`}>
        <div className={compact ? "space-y-3" : "lg:col-span-7 space-y-4"}>
        ...
        <div className={compact ? "rounded-xl border border-white/10 bg-black/70 overflow-hidden" : "lg:col-span-5 rounded-xl border border-white/10 bg-black/70 overflow-hidden"}>
